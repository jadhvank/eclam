...
/// `eclam status [--json]` — read-only. Does not touch XPC.
/// ADR-0007 §C.
enum StatusCommand: CLISubcommand {
    static func run(args: [String]) -> Int32 {
        let json = args.contains("--json")

        let helperStatus = readHelperStatus()
        let sleepDisabled = readSleepDisabled()
        let agentMode = readAgentMode()
        let watched = readWatchedAgents()
        // v0.3.2 — only attempt the XPC roundtrip when the helper is enabled.
        // Falls back to the empty array (not the JSON `null`) so the field
        // shape stays stable for downstream consumers.
        let activeAgents: [String] = (helperStatus == "enabled")
            ? (readActiveAgentsViaXPC() ?? [])
            : []
        // ADR-0025 — CLI TTL hold 잔여 (-1 forever / 0 none / >0 sec / nil unknown).
        let holdRemaining: Double? = (helperStatus == "enabled") ? readHoldViaXPC() : nil
    ...
    private static func readHelperStatus() -> String {
