...
- **ログイン時に起動(任意)** — ログインすると Electronic Clam を自動で起動します。デフォルトはオフです。
- **アップデート通知** — GitHub で新しいリリースを確認し、ダウンロード先を案内します。通知するだけで、勝手にインストールすることはありません。
- **クラムシェル VPN ロック防止(任意・デフォルトはオフ)。** 外部ディスプレイなしのバッテリー駆動でフタを閉じると画面が*ロック*され、そのロックで FortiClient SSL VPN が切れて再ログインが必要になります。目に見えない仮想ディスプレイでセッションをつなぎ止めれば画面がロックされず、トンネルが維持されます — バックライトがないので電力はほぼゼロ、追加のハードウェアや電源も不要です。**画面だけオフ** の動作も **暗くする(Dim)**(暗いままロックしない・VPN 安全・デフォルト)と **スリープ(Sleep)** に分かれ、VPN が切れたら通知を受け取ることもできます。
- **より堅牢なヘルパー設定** — 検疫(quarantine)されたダウンロードや、macOS が登録を拒む一時的(translocation)な場所からはバックグラウンドヘルパーを登録せず、先にアプリを Applications へ移すよう案内します。設定は重複コピーとバージョンの不一致を指摘し、`eclam repair` が動かなくなった/到達できないヘルパーを復旧します。
